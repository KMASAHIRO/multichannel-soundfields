...
[シーンを自作する場合](#シーンを自作する場合)や[AIでシーンを生成する場合](#AIでシーンを生成する場合)は、送信機位置及び送信機の向きの座標はBlender上のXYZ座標に対応しています。
...
[シーンを自作する場合](#シーンを自作する場合)や[AIでシーンを生成する場合](#AIでシーンを生成する場合)は、受信機位置及び受信機の向きの座標はBlender上のXYZ座標に対応しています。
...
各`tx_i/rx_j.npz`は、送信機のインデックス`i`と受信機のインデックス`j`の組合せに対するインパルス応答データです。
...
[シミュレーション](#シミュレーション)実行時はXMLのパスを指定します。シミュレーション時にPLYファイルも参照するため、もしエクスポートしたファイルを移動させる場合は、ディレクトリ構成を保ってください。
...
以下の手順を行うにあたり、AcoustiXシミュレーションとは別の環境を用意してください。
